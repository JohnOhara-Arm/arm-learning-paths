...
`sum=1000000000`. The `baseline-c2c.txt` file was created in the previous
step, and `padded-c2c.txt` was created above. Use the same AWK match for both
...
counter line. **Please Note**: It is not, by itself, proof that either address belongs to the
counter object.

The two addresses do not need to match. They come from separate JVM processes.
